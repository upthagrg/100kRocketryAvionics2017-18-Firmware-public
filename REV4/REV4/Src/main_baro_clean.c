...
int main(void)
{
  ...
  SystemClock_Config();
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
    cmd = 0x1E;
    spi_send8(SPI3, &cmd, &baro_reset); //Send baro reset command 0x1E and write status of send to baro_reset
    HAL_Delay(15); //allow time to reset
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET); //pull high to finish
    cmd = 0xA0; //ADC PROM Read;
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);
    spi_send8(SPI3, &cmd, &baro_reset);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET); //pull high to finish
    HAL_Delay(15);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET);//pull NSS low to start
  ...
  {  
	    ...
	    sprintf(Test, "TELEM: PRESSURE: %d | %d | %d EOT!\n\r", arr[0], arr[1], arr[2]); //format in returned data

	    HAL_UART_Transmit(&huart5, (unsigned char*)Test2, sizeof(Test2), HAL_MAX_DELAY); //send status
	    HAL_Delay(500);
	    GPIOC->ODR = 0x0000FFFF;
	    HAL_Delay(500);
	    GPIOC->ODR = 0x00000000;
