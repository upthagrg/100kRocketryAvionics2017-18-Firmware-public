...
HAL_StatusTypeDef baro_reset, baro_convert, baro_write1, baro_write2, baro_write3, baro_write4, baro_read1, baro_read2, baro_read3, baro_read4 = 5; //USER DECLARED GLOBALS
...
struct baro_data{
	uint8_t p1; //pressure bits
	uint8_t p2; //pressure bits
	uint8_t p3; //pressure bits
	uint8_t t1; //temp bits
	uint8_t t2; //temp bits
	uint8_t t3; //temp bits
};
...
void spi_send8(SPI_TypeDef* spi, uint8_t* cmdbyref, HAL_StatusTypeDef* in)
{
	char Test[128];
	memset(Test, '\0', 128);
    sprintf(Test, "cmd in spi_send8 = %d EOT!\n\r", *cmdbyref);

	  HAL_UART_Transmit(&huart5, (unsigned char*)Test, sizeof(Test), HAL_MAX_DELAY); //send data
	  HAL_Delay(500);
      HAL_Delay(500);

	//uint8_t data2 = 0;
	//*in = HAL_SPI_TransmitReceive(&hspi3, &data, &data2, 1, 0xFFFFFFFF);

	*in = HAL_SPI_Transmit(&hspi3, cmdbyref, 1, 0xFFFFFFFF);
}
...
uint8_t spi_read8(SPI_TypeDef* spi, HAL_StatusTypeDef* in)
{
	...
	char Test[128];
	memset(Test, '\0', 128);
	sprintf(Test, "data in spi_read8 before talk to spi = %d EOT!\n\r", data);
	...
	//*in = HAL_SPI_TransmitReceive(&hspi3, &data2, &data, 1, 0xFFFFFFFF);
	//*in = HAL_SPI_TransmitReceive(&hspi3, &data, &data2, 1, 0xFFFFFFFF);
	*in = HAL_SPI_Receive(&hspi3, &data, 1, 0xFFFFFFFF);

	memset(Test, '\0', 128);
	sprintf(Test, "data in spi_read8 after talk to spi = %d EOT!\n\r", data);
...
struct baro_data read_baro(){
	...
	spi_send8(SPI3, &cmd, &baro_convert); 					//Send conversion command over SPI3; write status of send to baro_convert
	...
	//spi_send8(SPI3, &cmd, &baro_convert); this statement will confirm that you sent 0x00
	HAL_SPI_TransmitReceive(&hspi3, &cmd, arr, 3, 0xFFFFFFFF);
	//spi_send8(SPI3, cmd, &baro_write1); //may need or not, depends on example seen
...
//	spi_send(0x00);
	arr[1] = SPI3->DR;
	//a.p2 = spi_read8(SPI3, &baro_read2);
	spi_send8(SPI3, cmd, &baro_write4);
//	spi_send(0x00);
	arr[2] = SPI3->DR;
	//a.p3 = spi_read8(SPI3, &baro_read3);
*/
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET);
...
//HAL_StatusTypeDef HAL_I2C_Master_Transmit(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint8_t *pData, uint16_t Size, uint32_t Timeout)
	...
	HAL_I2C_Master_Transmit(&hi2c2, 0x69, &reg, 1, 0xFFFFFFFF);
	HAL_I2C_Master_Receive(&hi2c2, 0x69, &data, 1, 0xFFFFFFFF);
...
REMOVED FOR THE TIME BEING UNTIL GLENN GIVES APPROVAL
*************************************************/
...
int main(void)
{
  ...
  SystemClock_Config();
    ...
    HAL_Delay(60000);
    //memset(GPSTEST, '\0',10);
    //sprintf(GPSTEST, "PMTK225,0");
    //HAL_UART_Transmit(&huart4, (unsigned char*)GPSTEST, 8, HAL_MAX_DELAY);
    //uret = huart4.Instance->RDR;
    //sprintf(Test, "TELEM: UART4 RDR: %d EOT!\n\r", uret);
    //HAL_UART_Transmit(&huart5, (unsigned char*)Test, sizeof(Test), HAL_MAX_DELAY);
    //HAL_Delay(60000);
  ...
  while (1)
  {
	  ...
	  memset(GPSTEST, '\0',8);
	  memset(GPSTEST2, '\0',1024);
	  sprintf(GPSTEST, "PMTK000");
	  //dataret = read_baro(); //gets baro data
	  //format in status returns
	  //sprintf(Test2, "TELEM: BARO STATUS: %d | %d | %d | %d | %d | %d | %d | %d | %d EOT!\n\r", baro_reset, baro_convert, baro_write1, baro_write2, baro_write3, baro_write4, baro_read1, baro_read2, baro_read3); //fills the buffer struct to char pointer
	  //sprintf(Test, "TELEM: PRESSURE: %d | %d | %d EOT!\n\r", dataret.p1, dataret.p2, dataret.p3); //format in returned data

	  //HAL_UART_Receive(&huart4, (uint8_t*)(&get[0]), 1, HAL_MAX_DELAY);
	  //strcat(buff, get);
	  //HAL_UART_Transmit(&huart5, (unsigned char*)Test2, sizeof(Test2), HAL_MAX_DELAY);
	  HAL_Delay(500);
	  HAL_UART_Transmit(&huart4, (unsigned char*)GPSTEST, 8, HAL_MAX_DELAY);
      HAL_Delay(500);
	  uret2 = huart4.Instance->TDR;
	  sprintf(Test2, "TELEM: UART4 TDR: %d EOT!\n\r", uret2);
	  HAL_UART_Transmit(&huart5, (unsigned char*)Test2, sizeof(Test2), HAL_MAX_DELAY);
	  HAL_Delay(500);
	  GPIOC->ODR = 0x0000FFFF;
	  //uret = huart4.Instance->RDR;
      //sprintf(Test, "TELEM: UART4 RDR: %d EOT!\n\r", uret); //format in returned data
	  HAL_UART_Receive(&huart4, (unsigned char*)GPSTEST2, sizeof(GPSTEST2), HAL_MAX_DELAY);
	  HAL_Delay(500);
	  GPIOC->ODR = 0x00000000;
	  HAL_UART_Transmit(&huart5, (unsigned char*)Test, sizeof(Test), HAL_MAX_DELAY);
	  HAL_Delay(500);
